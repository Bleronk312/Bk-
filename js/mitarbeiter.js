...
function appRerenderDetail() { if (currentScheine) renderDetail(); }
let sigPad = null;
...
// Beim Tippen wird nur die Trefferliste unterhalb neu gebaut - Fokus/Tastatur bleiben stabil.
function onScheineSearchInput(value) {
  scheineSearchQuery = value;
  const box = document.getElementById("scheineListResults");
  if (box) box.innerHTML = renderListResults();
  else renderList();
}
